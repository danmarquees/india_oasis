document.addEventListener("DOMContentLoaded", () => {
  ...
  function addReviewFormListeners() {
    ...
    if (reviewForm) {
      ...
      const starRatingRadioContainer = reviewForm.querySelector(".star-rating-radio");
      const radioButtons = starRatingRadioContainer ? starRatingRadioContainer.querySelectorAll('input[type="radio"]') : [];
      const starLabels = starRatingRadioContainer ? starRatingRadioContainer.querySelectorAll('label i') : [];
      ...
      starLabels.forEach(starIcon => {
        ...
        const initialRatingInput = Array.from(radioButtons).find(radio => radio.checked);
      ...
      reviewForm.addEventListener("submit", async (e) => {
        ...
        if (!rating) {
          showFeedback("Erro", "Por favor, selecione uma nota para a sua avaliação.", "error");
          return;
        }
        ...
        try {
          ...
          if (response.ok) { // Check for successful HTTP status (200-299)
            ...
            if (productReviewsContainer) {
              ...
              productReviewsContainer.insertAdjacentHTML('afterbegin', newReviewHtml);

              // Render stars for the newly added review
              const newReviewElement = productReviewsContainer.firstElementChild;
              const newReviewStarContainer = newReviewElement ? newReviewElement.querySelector('.star-rating') : null;
            ...
            const mainProductRatingStarsContainer = document.querySelector('#product-rating .star-rating');
            const reviewSummaryAverageRatingSpan = document.querySelector('.mt-12.bg-white .text-4xl.font-bold.text-primary');
            const reviewSummaryStarsContainer = document.querySelector('.mt-12.bg-white .star-rating');
            const productReviewsCountSpans = document.querySelectorAll('#product-reviews-count'); // Select all elements with this ID

            if (reviewSummaryAverageRatingSpan) {
              reviewSummaryAverageRatingSpan.textContent = data.average_rating.toFixed(1);
            }
            ...
            const alreadyReviewedMessageDiv = document.querySelector('.mt-8 .bg-blue-100');
            if (alreadyReviewedMessageDiv) {
                alreadyReviewedMessageDiv.style.display = 'block';
            }


          } else { // Handle non-2xx responses
            showFeedback("Erro", data.message || "Ocorreu um erro ao enviar sua avaliação.", "error");
            if (data.errors) {
            ...
            if (data.message && data.message.includes('Você já avaliou este produto')) {
                reviewForm.style.display = 'none';
                const alreadyReviewedMessageDiv = document.querySelector('.mt-8 .bg-blue-100');
                if (alreadyReviewedMessageDiv) {
                    alreadyReviewedMessageDiv.style.display = 'block';
          ...
          showFeedback("Erro", "Não foi possível conectar ao servidor. Tente novamente mais tarde.", "error");
        }
      });
    }
  }
